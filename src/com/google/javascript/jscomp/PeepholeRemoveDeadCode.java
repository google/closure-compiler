...
class PeepholeRemoveDeadCode extends AbstractPeepholeOptimization {
  ...
  private Node removeSwitchWithSingleCase(Node n) {
    Node caseBlock = n.getLastChild().getLastChild();
    Node lastStatementToInclude = null;
    for (Node prev = null, statement = caseBlock.getFirstChild(); statement != null;
        statement = statement.getNext()) {
      if (isExit(statement)) {
        if (statement.isBreak() && !statement.hasChildren()) {
          lastStatementToInclude = prev;
          statement.detachFromParent();
        } else {
          lastStatementToInclude = statement;
        }
        break;
      }
      prev = statement;
    ...
    for (Node statement: caseBlock.children()) {
      block.addChildToBack(statement.detachFromParent());
      if (statement == lastStatementToInclude) {
        break;
      }
    }
  ...
  private Node tryRemoveSwitch(Node n) {
    if (n.hasOneChild()) {
      ...
      return removeSwitchWithSingleCase(n);
    } else {
      return null;
    }
  }
    ...
    if (defaultCase == null || n.getLastChild().isDefaultCase()) {
      ...
      if (NodeUtil.isLiteralValue(cond, false)) {
        ...
        if (caseMatches != TernaryValue.UNKNOWN) {
          ...
          // If there is one case left, we are able to fold it
          cur = cond.getNext();
