 ...
 * run, such as removal of casts or assertions. This pass must run after those optimizations,
 * {@link ProcessClosurePrimitives}, {@link Es6ConvertSuper}, and type checking.
 */
public final class RemoveSuperMethodsPass implements CompilerPass {
  ...
  private class RemoveSuperMethodsCallback extends AbstractPostOrderCallback {
      ...
      if (n.isFunction()
        ...
        if ((statement.isExprResult() || statement.isReturn())
          ...
          if (argumentsMatch(n, call)
              && returnMatches(call)
              && functionNameMatches(functionName, call)) {
     ...
     * the superclass.
     * @param enclosingFunctionName qualified name of the enclosing function.
     * @param call the CALL node inside the function.
     */
    private boolean functionNameMatches(String enclosingFunctionName, Node call) {
      String callName = call.getFirstChild().getQualifiedName();
      if (callName == null) {
        return false;
      }
      // The enclosing function name will have the form Foo.prototype.bar
      // The CALL should either be Foo.superClass_.bar.call or FooBase.prototype.bar.call
      String shortFunctionName =
          enclosingFunctionName.substring(enclosingFunctionName.lastIndexOf('.') + 1);
      if (callName.endsWith(DOT_JOINER.join(SUPERCLASS_MARKER, shortFunctionName, "call"))) {
        // matched superClass_ marker generated by ProcessClosurePrimitives. No further
        // name checks needed.
        return true;
      }
      if (!callName.endsWith(DOT_JOINER.join(PROTOTYPE_MARKER, shortFunctionName, "call"))) {
        return false;
      }
      // Check that call references the superclass
      String subclass =
          enclosingFunctionName.substring(0, enclosingFunctionName.indexOf(PROTOTYPE_MARKER));
      String calledClass = callName.substring(0, callName.indexOf(PROTOTYPE_MARKER));
      JSType subclassType = compiler.getTypeIRegistry().getType(subclass);
      JSType calledClassType = compiler.getTypeIRegistry().getType(calledClass);
      if (subclassType == null || calledClassType == null) {
        return false;
      }
      return subclassType.getLeastSupertype(calledClassType) == calledClassType;
    }
    ...
    /**
     * Returns true if the arguments of the call are the parameters of the enclosing function
     */
    private boolean argumentsMatch(Node enclosingFunction, Node call) {
