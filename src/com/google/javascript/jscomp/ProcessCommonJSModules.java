...
public final class ProcessCommonJSModules implements CompilerPass {
  ...
  public static final DiagnosticType UNKNOWN_REQUIRE_ENSURE = DiagnosticType.warning(
      "JSC_COMMONJS_UNKNOWN_REQUIRE_ENSURE_ERROR",
      "Unrecognized require.ensure call: {0}");

  public static final DiagnosticType SUSPICIOUS_EXPORTS_ASSIGNMENT = DiagnosticType.warning(
  ...
  /**
   * Module rewriting is done a on per-file basis prior to main
   * compilation. The pass must handle ES6+ syntax and the root
   * node for each file is a SCRIPT - not the typical jsRoot of
   * other passes.
   */
  @Override
  public void process(Node externs, Node root) {
    ...
    NodeTraversal.traverseEs6(compiler, root,
        new RewriteModule(finder.isCommonJsModule(), exports.build()));
  }

  /**
   * Information on a Universal Module Definition
   * A UMD is an IF statement and a reference
   * to which branch contains the commonjs export
   */
  ...
  /**
   * Traverse the script. Find all references to CommonJS
   * require (import) and module.exports or export statements.
   * Add goog.require statements for any require statements.
   * Rewrites any require calls to reference the rewritten module name.
   */
  class FindImportsAndExports extends AbstractPreOrderCallback {
    ...
    public void visit(NodeTraversal t, Node n, Node parent) {
      ...
      if (n.isName() && EXPORTS.equals(n.getString())) {
        ...
        if (v == null || v.isGlobal()) {
          ...
          if (qNameRoot != null && EXPORTS.equals(qNameRoot.getQualifiedName()) && NodeUtil.isLValue(qNameRoot)) {
    ...
    private void visitRequireCall(NodeTraversal t, Node require, Node parent) {
      ...
      if (imports.add(moduleName)) {
        ...
        this.script.addChildToFront(IR.exprResult(
     ...
     * Require.ensure must always be of the form:
     *
     *   require.ensure(['module1', ...], function(require) {})
     */
    private void visitRequireEnsureCall(NodeTraversal t, Node call) {
      if (call.getChildCount() != 3) {
        compiler.report(t.makeError(call, UNKNOWN_REQUIRE_ENSURE,
            "Expected the function to have 2 arguments but instead found {0}", "" + call.getChildCount()));
        return;
      }

      Node dependencies = call.getSecondChild();
      if (!dependencies.isArrayLit()) {
        compiler.report(t.makeError(dependencies, UNKNOWN_REQUIRE_ENSURE,
      ...
      for (Node dep : dependencies.children()) {
        if (!dep.isString()) {
          compiler.report(t.makeError(dep, UNKNOWN_REQUIRE_ENSURE,
              "The first argument must be an array literal of string literals."));
          return;
        }
      }
      ...
      if (!(callback.isFunction()
        ...
        compiler.report(t.makeError(callback, UNKNOWN_REQUIRE_ENSURE,
    ...
    /**
     * Add a goog.provide statement for the module. If the export is
     * directly assigned more than once, or the assignments are not global,
     * declare the module name variable.
     *
     * If all of the assignments are simply property assignments, initialize
     * the module name variable as a namespace.
     */
    void addGoogProvide() {
      ...
      this.script.addChildToFront(IR.exprResult(
          IR.call(IR.getprop(IR.name("goog"), IR.string("provide")),
              IR.string(moduleName))).useSourceInfoIfMissingFromForTree(this.script));
      ...
      if (directAssignmentsAtTopLevel > 1 || (directAssignmentsAtTopLevel == 0 && directAssignments > 0) ||
          directAssignments == 0) {
    ...
    /**
     * Find the outermost if node ancestor for a node without
    ...
    /**
     * Remove a Universal Module Definition and leave
     * just the commonjs export statement
     */
  ...
  /**
   * Traverse a file and rewrite all references to imported
   * names directly to the targeted module name.
   *
   * If a file is a CommonJS module, rewrite export statements.
   * Typically exports create an alias - the rewriting tries to
   * avoid such aliases.
   */
  private class RewriteModule extends
    ...
    public void visit(NodeTraversal t, Node n, Node parent) {
      switch(n.getToken()) {
        ...
        case STRING_KEY: {
          if (n.hasChildren() || n.isQuotedString() || n.getParent().getParent().isDestructuringLhs()) {
            break;
          }
    ...
    /**
     * Visit require calls. Rewrite require statements
     * to be a direct reference to name of require module. By this point
     * all references to the import alias should have already been renamed.
     */
    private void visitRequireCall(NodeTraversal t, Node require, Node parent) {
      String requireName = require.getSecondChild().getString();
      ModulePath modulePath = t.getInput().getPath().resolveCommonJsModule(requireName);
    ...
    /**
     * Visit export statements. Export statements can be
     * either a direct assignment:
     *    module.exports = foo
    ...
    private void visitExport(NodeTraversal t, Node export) {
      ...
      if ("module.exports".equals(root.getQualifiedName())){
        if (rValue != null && rValue.isObjectLit() &&
          root.getParent().isAssign() && root.getParent().getParent().isExprResult()) {
      ...
      String moduleName = modulePath.toModuleName();
      Node updatedExport = NodeUtil.newName(compiler, moduleName, export, export.getQualifiedName());

      // Ensure that direct assignments to "module.exports" have var definitions
      if ("module.exports".equals(root.getQualifiedName()) && rValue != null &&
          t.getScope().getVar("module.exports") == null && root.getParent().isAssign() &&
          root.getParent().getParent().isExprResult()) {
    ...
    /**
     * Since CommonJS modules may have only a single export, it's
     * common to see the export be an object literal.  We want to
     * expand this to individual property assignments. If any individual
    ...
    private void expandObjectLitAssignment(NodeTraversal t, Node export) {
      ...
      while (key != null) {
        ...
        Node expr = IR.exprResult(IR.assign(lhs, value))
            .useSourceInfoIfMissingFromForTree(key);
     ...
     *   1. References to an import alias. These are replaced with
     *      a direct reference to the imported module.
     *   2. Names which are exported. These are rewritten to be the
     *      export assignment directly.
     *   3. Global names: If a name is global to the script, add
     *      a suffix so it doesn't collide with any other global.
     *
     *  Rewriting case 1 is safe to perform on all files. Cases 2 and 3
     *  can only be done if this file is a commonjs module.
     */
     ...
     * @param requireFunctionExpressions Whether named class or functions should be rewritten
     *     to variable assignments
     */
    private void updateNameReference(NodeTraversal t, Node nameRef, String originalName, String newName,
      ...
      switch (parent.getToken()) {
        ...
        case CONST:
          if (newNameIsQualified) {
            ...
            if (nameRef.hasChildren()) {
              Node expr = IR.exprResult(IR.assign(getProp, nameRef.getFirstChild().detachFromParent()))
        ...
        case GETPROP: {
          Node name = newNameIsQualified ? NodeUtil.newQName(compiler, newName, nameRef, originalName) :
     ...
     * @return string - If the name is not used in an export, return it's own name
     *   If the name node is actually the export target itself, return
     *   null;
     */
        ...
        if (rValue.isObjectLit()) {
          ...
          if (qName != null) {
            ...
            if (exportVar != null && exportVar.getNode() == var.getNode()) {
              String exportPrefix = exportedNameBase.startsWith(MODULE) ? "module.exports" : EXPORTS;
    ...
    /**
     * Determine if the given Node n is an alias created by a
     * module import.
     *
    ...
    /**
     * Update any type references in JSDoc annotations to account for
     * all the rewriting we've done.
     */
