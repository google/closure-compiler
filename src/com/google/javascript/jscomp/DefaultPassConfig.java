...
public final class DefaultPassConfig extends PassConfig {
  ...
  protected List<PassFactory> getChecks() {
    ...
    // NOTE(dimvar): This will move later into the compilation as we convert
    // optimization passes to handle types generated by the new type inference.
    // For now, the plan is to move it into the optimizations. It's possible
    // that some optimization pass will rewrite the code to a state that's not
    // suitable for type checking, and we won't be able to move past that pass.
    // Possible options then are:
    // - Convert the remaining optimizations to use TypeI and test that only
    //   in unit tests, not full builds. Once all passes are converted, then
    //   drop the OTI-after-NTI altogether.
    // - Retain types from both type systems on the AST, and passes can choose
    //   which types to get. This will blow up memory usage, but it might be
    //   more promising because we need to be testing full builds with new types
    if (options.getNewTypeInference()) {
      addOldTypeCheckerPasses(checks, options);
    }
