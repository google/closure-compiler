...
class RemoveUnusedPolyfills implements CompilerPass {
  ...
  private class Traverser extends AbstractPostOrderCallback {
    ...
    final SetMultimap<PrototypeMethod, Node> methodPolyfills = HashMultimap.create();
    final SetMultimap<String, Node> staticPolyfills = HashMultimap.create();
    ...
    void visitPolyfillDefinition(Node node, String polyfill) {
      ...
      if (method != null) {
        methodPolyfills.put(method, node);
        methodsByName.put(method.method, method);
      } else {
        staticPolyfills.put(polyfill, node);
      }
    }
    ...
    void visitPossiblePolyfillUse(Node node) {
      ...
      if (node.isQualifiedName()) {
        ...
        staticPolyfills.removeAll(qname);
        methodPolyfills.removeAll(PrototypeMethod.split(qname));
      }
    ...
    private void checkType(
      ...
      if (!targetType.getGreatestSubtype(type).isBottom()) {
        methodPolyfills.removeAll(method);
      }
    }
    ...
    public void visit(NodeTraversal traversal, Node node, Node parent) {
      if (NodeUtil.isExprCall(node)) {
        ...
        if ("$jscomp.polyfill".equals(originalName) || "$jscomp.patch".equals(originalName)) {
