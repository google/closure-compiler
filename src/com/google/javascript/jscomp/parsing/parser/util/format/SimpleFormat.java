...
public final class SimpleFormat {
        ...
        while (i < length) {
            ...
            if (i < length) {
                ...
                if (token.requireArgument()) {
                    int index = token.getArgIndex() == FormatToken.UNSET
                        ? currentObjectIndex++
                        : token.getArgIndex();
    ...
    private static class FormatToken {
        ...
        boolean isDefault() {
            return !flagAdd && !flagComma && !flagMinus
                && !flagParenthesis && !flagSharp
                && !flagSpace && !flagZero
                && width == UNSET && precision == UNSET;
        }
...
//                }
    ...
    private IllegalFormatConversionException badArgumentType() {
        throw new IllegalFormatConversionException(
            formatToken.getConversionType(), arg.getClass());
    }
    ...
    private void transform_e(StringBuilder result) {
        ...
        */
        String formattedString = arg.toString(); // decimalFormat.format(arg);
    ...
    private void transform_g(StringBuilder result) {
        ...
        // Skipp a bunch of scientific notation stuff.
        transform_f(result);
    }
