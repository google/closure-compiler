...
public final class CheckPathsBetweenNodes<N, E> {
   ...
   *
   * <p>Note that nodePredicate is not necessarily called for all nodes in G nor is edgePredicate
   * called for all edges in G.
   *
   * @param graph Graph G to analyze.
   * @param a The node A.
   * @param b The node B.
   * @param nodePredicate Predicate which at least one node on each path from an A node to B
   *     (inclusive) must match.
   * @param edgePredicate Edges to consider as part of the graph. Edges in graph that don't match
   *     edgePredicate will be ignored.
   * @param inclusive Includes node A and B in the test for the node predicate.
   */
  public CheckPathsBetweenNodes(
      DiGraph<N, E> graph,
      DiGraphNode<N, E> a,
      DiGraphNode<N, E> b,
