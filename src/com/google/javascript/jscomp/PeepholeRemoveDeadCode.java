...
class PeepholeRemoveDeadCode extends AbstractPeepholeOptimization {
  ...
  private Node tryOptimizeSwitch(Node n) {
    ...
    if (n.hasOneChild()) {
      ...
      reportCodeChange();
      return replacement;
    } else if (n.getChildCount() == 2 && defaultCase != null) {
      // Remove the switch if the default case is the only one remaining
      Node defaultCaseBlock = defaultCase.getFirstChild();
      Node lastStatement = defaultCaseBlock.getLastChild();
      if (lastStatement.isBreak() && !lastStatement.hasChildren()) {
      ...
      for (Node statement : defaultCaseBlock.children()) {
