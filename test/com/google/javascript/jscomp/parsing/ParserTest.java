...
public final class ParserTest extends BaseJSTypeTestCase {
  ...
  public void optionalChainingGetProp() {
    ...
    assertNode(n).hasType(Token.GETPROP);
    assertNode(n).isOptionalChain();
    assertNode(n).hasLineno(1);
    assertNode(n).hasCharno(1);
    assertNode(n.getFirstChild()).isEqualTo(IR.name("a"));
  ...
  public void optionalChainingGetElem() {
    ...
    assertNode(n).hasType(Token.GETELEM);
    assertNode(n).isOptionalChain();
    assertNode(n).hasLineno(1);
    assertNode(n).hasCharno(1);
    assertNode(n.getFirstChild()).isEqualTo(IR.name("a"));
  ...
  @Test
  public void optionalChainingGall() {
    mode = LanguageMode.UNSUPPORTED;
    Node n = parse("a?.()").getFirstFirstChild();

    assertNode(n).hasType(Token.CALL);
    assertNode(n).isOptionalChain();
    assertNode(n).hasLineno(1);
    assertNode(n).hasCharno(1);
    assertNode(n.getFirstChild()).isEqualTo(IR.name("a"));
  ...
  public void optionalChainingChain() {
    ...
    parse("a?.b?.c?.[1]");
    parse("a?.b?.c?.()");
    // make sure can handle call with param
    parse("a?.(c)?.b");
  }
  ...
  public void optionalChainingMiscErrors() {
    ...
    parseError("super?.()", "Optional chaining not allowed here.");
    parseError("super?.foo", "Optional chaining not allowed here.");
    parseError("new?.target", "Optional chaining not allowed here.");
    parseError("import?.('foo')", "Optional chaining not allowed here.");
  }
