...
public final class CheckMissingTrailingComma extends AbstractPostOrderCallback
  ...
  public void visit(NodeTraversal t, Node n, Node parent) {
    ...
    if (n.getCharno() == Node.MAX_COLUMN_NUMBER
        || lastElement.getCharno() == Node.MAX_COLUMN_NUMBER) {
      // The column number was truncated, so we can't recover the exact position in the source.
      // As a consolation, any JavaScript file with lines over 4000 characters long is probably a
      // build artifact checked into source control, so it doesn't need linter findings anyway.
      return;
    }
    ...
    if (span.contains("\n") && !span.contains(",")) {
      t.report(lastElement, MISSING_TRAILING_COMMA);
    }
  }
}
