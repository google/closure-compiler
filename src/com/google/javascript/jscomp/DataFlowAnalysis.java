...
abstract class DataFlowAnalysis<N, L extends LatticeElement> {
  ...
  DataFlowAnalysis(ControlFlowGraph<N> cfg, JoinOp<L> joinOp) {
    ...
    Comparator<DiGraphNode<N, Branch>> optComparator = cfg.getOptionalNodeComparator(isForward());
    if (optComparator != null) {
      optComparator = comparingInt(this::getStepCount).thenComparing(optComparator);
    }
    this.workQueue = new UniqueQueue<>(optComparator);
  }
  ...
  private int getStepCount(DiGraphNode<N, Branch> node) {
