...
public final class Es6ConvertSuperConstructorCalls
  ...
  private void visitSuperCall(Node superNode, Node superCall) {
    ...
    newSuperCall.useSourceInfoFromForTree(superCall);
  ...
  private Node createSuperClassQNameNode(Node superCall) {
    ...
    JSDocInfo enclosingClassJsDoc = checkNotNull(NodeUtil.getBestJSDocInfo(enclosingConstructor));
    JSTypeExpression bangBaseType = checkNotNull(enclosingClassJsDoc.getBaseType());
    Node bangBaseTypeNode = bangBaseType.getRoot();
    Node baseTypeNode = bangBaseTypeNode.getFirstChild();
    String superClassName = baseTypeNode.getString();
    Node superClassQName = NodeUtil.newQName(compiler, superClassName);
