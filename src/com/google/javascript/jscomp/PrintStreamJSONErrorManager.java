...
/**
 * An error manager that prints error and warning data to the print stream
 * as an array of JSON objects provided in addition to the functionality of the
 * {@link BasicErrorManager}.
 */
public class PrintStreamJSONErrorManager extends BasicErrorManager {
   ...
   * @param sourceExcerptProvider used to retrieve the source context which
   *     generated the error
   */
  ...
  public void generateReport() {
    ...
    try (JsonWriter jsonWriter =
        new JsonWriter(new OutputStreamWriter(bufferedStream, "UTF-8"))) {
      ...
      for (ErrorWithLevel message = messages.poll(); message != null; message = messages.poll()) {
        ...
        String sourceExcerpt = SourceExcerpt.LINE.get(
            sourceExcerptProvider, sourceName, lineNumber, excerptFormatter);
        ...
        OriginalMapping mapping = sourceExcerptProvider.getSourceMapping(
      ...
      if (getTypedPercent() > 0.0) {
        summaryBuilder.append(SimpleFormat.format("%d error(s), %d warning(s), %.1f%% typed",
            getErrorCount(), getWarningCount(), getTypedPercent()));
      } else {
        summaryBuilder.append(SimpleFormat.format("%d error(s), %d warning(s)", getErrorCount(),
            getWarningCount()));
      }
      jsonWriter.beginObject();
