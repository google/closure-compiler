...
final class PolymerClassRewriter {
  ...
  void rewritePolymerClassDeclaration(
    ...
    // unless it was already declared in the constructor. It's unsafe to add a property to the
    // prototype if it has already been declared in the constructor, because we would clobber its
    // type information with the type generated by the PolymerPass.
  ...
  /**
   * Find the names of all class prototype properties declared in the body of a constructor.
   *
   * @param node The constructor function node to traverse.
   * @param names The set to add property names to.
   */
  private static void collectConstructorPropertyNames(Node node, Set<String> names) {
    ...
    for (Node child : node.children()) {
      if (child.isGetProp()
        ...
        // We found a "this.foo" expression. Add "foo".
